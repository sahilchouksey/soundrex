...
const searchQueryUrl = (searchQuery, searchTerm, rawParams) => {
  let url = `https://soundrex.herokuapp.com/api/v1/search?query=${searchQuery}`;
  if (searchTerm) {
...
const nextResultsUrl = ({ctoken, continuation, itct}) =>
  `https://soundrex.herokuapp.com/api/v1/search_next?ctoken=${ctoken}&continuation=${continuation}&type=next&itct=${itct}`;
